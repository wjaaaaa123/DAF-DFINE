...
class TRTInference(object):
    ...
    def _profile_opt_shape(self, engine, name):
        if hasattr(engine, "get_tensor_profile_shape"):
            return tuple(engine.get_tensor_profile_shape(name, 0)[1])
        return None

    def _resolve_binding_shape(self, engine, context, name, max_batch_size):
        shape = list(engine.get_tensor_shape(name))
        is_input = engine.get_tensor_mode(name) == trt.TensorIOMode.INPUT

        if is_input and any(dim < 0 for dim in shape):
            opt_shape = self._profile_opt_shape(engine, name)
            if opt_shape is not None:
                shape = [opt if dim < 0 else dim for dim, opt in zip(shape, opt_shape)]
            elif shape[0] < 0:
                shape[0] = max_batch_size
            context.set_input_shape(name, tuple(shape))

        context_shape = list(context.get_tensor_shape(name))
        if context_shape and all(dim >= 0 for dim in context_shape):
            shape = context_shape

        if any(dim < 0 for dim in shape):
            raise ValueError(f"Cannot resolve TensorRT binding shape for '{name}': {tuple(shape)}")
        return tuple(shape)

    def _resize_output_bindings(self):
        for name in self.output_names:
            shape = tuple(self.context.get_tensor_shape(name))
            if any(dim < 0 for dim in shape):
                raise ValueError(f"Cannot resolve TensorRT output shape for '{name}': {shape}")
            if self.bindings[name].shape != shape:
                data = torch.from_numpy(np.empty(shape, dtype=self.bindings[name].dtype)).to(self.device)
                self.bindings[name] = self.bindings[name]._replace(shape=shape, data=data, ptr=data.data_ptr())
                self.bindings_addr[name] = data.data_ptr()
    ...
    def get_bindings(self, engine, context, max_batch_size=32, device=None) -> OrderedDict:
        ...
        for i, name in enumerate(engine):
            shape = self._resolve_binding_shape(engine, context, name, max_batch_size)
