...
class TransformerEncoderLayer(nn.Module):
    def __init__(
        ...
        # 用 ConvolutionalGLU 替换原始 Linear FFN
    ...
    def forward(
        ...
        if size is None:
            raise ValueError(
                "ConvolutionalGLU requires spatial size=(H, W), "
                "but size is None."
            )
        ...
        if n != h * w:
            raise ValueError(
                f"Token length {n} does not match H*W={h*w}, "
                f"size=({h}, {w})."
            )

        # [B, HW, C] -> [B, C, H, W]
        src_2d = src.transpose(1, 2).reshape(
            b, c, h, w
        ).contiguous()

        src_2d = self.cglu(src_2d)

        # [B, C, H, W] -> [B, HW, C]
        src2 = src_2d.flatten(2).transpose(
            1, 2
        ).contiguous()
...
@register()
class HybridEncoderCGLU(nn.Module):
    ...
    def __init__(self,     
        ...
        super().__init__()
        print("[HybridEncoderCGLU] ConvolutionalGLU enabled.")
