...
from ..extre_module.custom_nn.upsample.DySample import DySample

#from ..extre_module.custom_nn.mlp.ConvolutionalGLU import ConvolutionalGLU

__all__ = ['HybridEncoderDySample']    
...
class TransformerEncoderLayer(nn.Module): 
    def __init__(self,  
        ...
        #self.cglu = ConvolutionalGLU(d_model, hidden_features=dim_feedforward,out_features=d_model)
    ...
    def forward(self, src, src_mask=None, pos_embed=None, size=None) -> torch.Tensor:  
        ...
        if self.normalize_before:
            src = self.norm2(src) 
        #src = src.transpose(2,1).reshape((src.size(0),size(2),size[0],src.size[1])) # B C H W 
        src = self.linear2(self.dropout(self.activation(self.linear1(src))))
        #src = self.cglu(src).flatten(2).permute(0, 2, 1) # B C H W -> B L(H*W) C
...
class TransformerEncoderBlock(nn.Module):
    ...
    def forward(self, src, src_mask=None) -> torch.Tensor:   
        ...
        residual = src
        if self.normalize_before:
            src = self.norm2(src) # B L(H*W) C -> B C L -> B C H W
        src = self.linear2(self.dropout(self.activation(self.linear1(src))))  
        src = residual + self.dropout2(src)
        if not self.normalize_before:    
            src = self.norm2(src)
...
class TransformerEncoder(nn.Module):    
    ...
    def forward(self, src, src_mask=None, pos_embed=None, size=None) -> torch.Tensor:
        ...
        for layer in self.layers:
            output = layer(output, src_mask=src_mask, pos_embed=pos_embed, size=None )   
...
@register()
class HybridEncoderDySample(nn.Module):
    ...
    def __init__(self,     
        ...
        for _ in range(len(in_channels) - 1, 0, -1):  # 从高层到低层遍历
            ...
            self.dysample_layers = nn.ModuleList([
                 DySample(in_channels=hidden_dim, scale=2, style='lp', groups=4, dyscope=False)
    ...
    def forward(self, feats):
       ...
       # FPN融合：使用DySample自顶向下融合
        ...
        for idx in range(len(self.in_channels) - 1, 0, -1):
            ...
            # 横向1×1卷积
            feat_heigh = self.lateral_convs[layer_idx](feat_heigh)
            inner_outs[0] = feat_heigh

            # 使用DySample替代nearest插值
            upsample_feat = self.dysample_layers[layer_idx](feat_heigh)

            # 检查上采样结果能否与低层特征拼接
            if upsample_feat.shape[-2:] != feat_low.shape[-2:]:
                raise RuntimeError(
                    "DySample output size does not match the low-level feature: "
                    f"upsample={upsample_feat.shape[-2:]}, "
                    f"low={feat_low.shape[-2:]}"
                )

            # 特征拼接与FPN融合
            inner_out = self.fpn_blocks[layer_idx](
                torch.concat(
