...
from ..misc.deformable_detr_msda import deformable_detr_msda_core_default, should_use_deformable_detr_msda
...
class MSDeformableAttention(nn.Module): 
    ...
    def forward(self,     
        ...
        if reference_points.shape[-1] == 2:
            ...
            rot = rot.reshape(bs, Len_q, reference_points.shape[2], 2, 2)
        ...
        if self.method == "default" and should_use_deformable_detr_msda(
            value,  
            sampling_locations,
            attention_weights,     
        ):
...
class TransformerDecoderLayer(nn.Module):     
    def __init__(self,
                 ...
                 use_flash_self_attn=False): # 保留旧配置兼容，运行时自动选择后端
        super(TransformerDecoderLayer, self).__init__()
        del use_flash_self_attn
