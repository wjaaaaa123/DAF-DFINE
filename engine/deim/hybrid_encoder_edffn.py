...
from ..extre_module.custom_nn.mlp.EDFFN import EDFFN
...
class EDFFNAdapter(nn.Module):
    """Convert Transformer tokens [B, HW, C] to feature maps [B, C, H, W]."""
    ...
    def forward(self, x, size):
        ...
        """
        if size is None:
            raise ValueError("EDFFN requires spatial size=(H, W).")
        ...
        if n != h * w:
            raise ValueError(
                f"Token number {n} does not match spatial size {h}x{w}."
            )
...
class TransformerEncoderLayer(nn.Module): 
    def __init__(self,  
        ...
        if self.use_edffn:
             self.edffn_adapter = EDFFNAdapter(
                 dim=d_model,
    ...
    def forward(self, src, src_mask=None, pos_embed=None, size=None) -> torch.Tensor:  
        ...
        if self.normalize_before:
            src = self.norm2(src) 
        if self.use_edffn:
            src2 = self.edffn_adapter(src, size)
        else:
...
@register()
class HybridEncoderEDFFN(nn.Module):
    ...
    def __init__(self,     
        ...
        super().__init__()
        print("[HybridEncoderEDFFN] EDFFN enabled.")
