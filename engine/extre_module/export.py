...
def get_dynamic_profile_shapes(input_names, shape, workspace):
    batch, _, height, width = shape    
    spatial_scale = int(max(2, workspace or 2))  
    max_h, max_w = spatial_scale * height, spatial_scale * width
    profiles = {}     
    for name in input_names: 
        if name in {"images", "rgb"}: 
            profiles[name] = ((1, 3, 32, 32), (batch, 3, height, width), (batch, 3, max_h, max_w))   
        elif name == "npy":
            profiles[name] = ((1, 1, 32, 32), (batch, 1, height, width), (batch, 1, max_h, max_w)) 
        elif name == "orig_target_sizes":  
            profiles[name] = ((1, 2), (batch, 2), (batch, 2))
    return profiles
...
def export_engine(
    ...
    if dynamic:
        ...
        profile_shapes = get_dynamic_profile_shapes([inp.name for inp in inputs], shape, workspace_gb)
        for inp in inputs:
            if inp.name in profile_shapes:    
                min_shape, opt_shape, max_shape = profile_shapes[inp.name]
