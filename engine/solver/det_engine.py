...
def _evaluator_update(evaluator, res, targets):  
    if evaluator is None:
        return
    try:    
        update_sig = inspect.signature(evaluator.update)
    except (TypeError, ValueError):
        evaluator.update(res)     
        return    
    if "targets" in update_sig.parameters: 
        evaluator.update(res, targets=targets)
    else: 
        evaluator.update(res)
 
   
def _runtime_tensor_from_samples(samples, name):
    if name in {"images", "image"}:    
        if isinstance(samples, dict) and name in samples:    
            value = samples[name]
        else:
            value = select_model_input_for_model(samples, model=None, key="rgb")
    elif name == "rgb":     
        if not isinstance(samples, dict) or "rgb" not in samples:
            raise KeyError("Runtime input 'rgb' requires multimodal samples with key 'rgb'.")  
        value = samples["rgb"]     
    elif name == "npy":
        if not isinstance(samples, dict) or "npy" not in samples:  
            raise KeyError("Runtime input 'npy' requires multimodal samples with key 'npy'.")  
        value = samples["npy"]
    else:
        raise KeyError(f"Unsupported runtime input name: {name}") 
    
    if not torch.is_tensor(value):   
        raise TypeError(f"Runtime input '{name}' must be a Tensor, got {type(value)}.")    
    return value  
    
 
def build_runtime_input_feed(input_names, samples, orig_target_sizes, for_onnx):     
    feed = {}     
    for name in input_names:
        if name == "orig_target_sizes": 
            tensor = orig_target_sizes.to(dtype=torch.float32)
        else:  
            tensor = _runtime_tensor_from_samples(samples, name)    
        feed[name] = tensor.detach().cpu().numpy() if for_onnx else tensor
    return feed  

    
def parse_runtime_outputs(outputs_by_name):
    labels = outputs_by_name["labels"]
    boxes = outputs_by_name["boxes"]   
    scores = outputs_by_name["scores"]
    masks = outputs_by_name.get("masks") 

    def as_tensor(value):
        return torch.from_numpy(value) if isinstance(value, np.ndarray) else value
  
    labels, boxes, scores = as_tensor(labels), as_tensor(boxes), as_tensor(scores)
    masks = as_tensor(masks) if masks is not None else None  
    ...
    if masks is None:     
        ...
        for lab, box, sco, mask in zip(labels, boxes, scores, masks):
            results.append(dict(labels=lab, boxes=box, scores=sco, masks=mask)) 
    return results
     
   
def _tensor_has_nonfinite(value):
    return torch.is_tensor(value) and value.is_floating_point() and not torch.isfinite(value).all()   
   
   
def _outputs_nonfinite_reason(outputs, keys=("pred_logits", "pred_boxes")):  
    if not isinstance(outputs, dict):
        return None
    for key in keys:
        value = outputs.get(key)   
        if _tensor_has_nonfinite(value):
            return f"{key} contains NaN/Inf"
    return None

  
def _loss_dict_nonfinite_reason(loss_dict):
...
def _first_nonfinite_floating_buffer(model, ignore_names=("decoder.anchors",)):
    ignored = set(ignore_names)     
    for name, buffer in model.named_buffers():
        clean_name = name.replace("module.", "", 1)
        if clean_name in ignored:
            continue    
        if _tensor_has_nonfinite(buffer):
            return name
    return None


def _sync_nonfinite_failure(reason, device):    
    local_failed = reason is not None
    if dist_utils.is_dist_available_and_initialized():
        flag = torch.tensor([1 if local_failed else 0], device=device, dtype=torch.int32)    
        torch.distributed.all_reduce(flag, op=torch.distributed.ReduceOp.MAX) 
        return bool(flag.item()), reason    
    return local_failed, reason

 
def _format_nonfinite_failure(reason, epoch, step):
    if reason is None:  
        reason = "another distributed rank reported NaN/Inf"
    return {    
        "_amp_epoch_failed": True,
        "_failed_epoch": epoch,
        "_failed_step": step,     
...
def _handle_nonfinite_failure(reason, epoch, step, device, use_amp_fallback_fp32, force_fp32):  
    failed, synced_reason = _sync_nonfinite_failure(reason, device)     
    if not failed:
        return None   
     
    message = "Non-finite training state at epoch={} step={}: {}".format(
        epoch,
        step,     
        synced_reason or "another distributed rank reported NaN/Inf",  
    )
    if use_amp_fallback_fp32 and not force_fp32:  
        logger.warning(message + "; aborting this epoch for FP32 retry.")
        return _format_nonfinite_failure(synced_reason, epoch, step)    
...
def train_one_epoch(self_lr_scheduler, lr_scheduler, model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    swanlab = kwargs.get('swanlab', None)  
    ...
    force_fp32 = kwargs.get('force_fp32', False)    
    ...
    for i, (samples, targets) in pbar:
        ...
        allow_amp_epoch_retry = scaler is not None and not force_fp32 and use_amp_fallback_fp32 
     
        # 使用混合精度训练   
        if scaler is not None and not force_fp32:
            with dt[1]:
                with torch.autocast(device_type=str(device), cache_enabled=True):    
                    outputs = model(model_inputs, targets=targets) 

            failure = _handle_nonfinite_failure(  
                _outputs_nonfinite_reason(outputs), 
                epoch,
                i,  
                device,   
                allow_amp_epoch_retry,    
                force_fp32,   
            )
            ...
            failure = _handle_nonfinite_failure(
                _loss_dict_nonfinite_reason(loss_dict) or ("loss contains NaN/Inf" if _tensor_has_nonfinite(loss) else None),
                epoch,
                i,
                device,   
                allow_amp_epoch_retry,
            ...
            if scaler is not None and force_fp32:   
                with dt[1]:     
                    outputs = model(model_inputs, targets=targets)
            else:  
                with dt[1]:  
        ...
        with dt[4]:
            bad_buffer = _first_nonfinite_floating_buffer(model)
            failure = _handle_nonfinite_failure(     
                f"model buffer {bad_buffer} contains NaN/Inf" if bad_buffer else None,
                epoch,  
                i,   
                device,
                allow_amp_epoch_retry,
            ...
            if swanlab and dist_utils.is_main_process() and global_step % 10 == 0:   
                swanlab_payload = {     
                    'train/loss/total': _to_log_scalar(loss_value),
                    'train/epoch': epoch,  
                    'train/step': i,    
                }   
                for j, pg in enumerate(optimizer.param_groups):
                    swanlab_payload[f'train/lr/pg_{j}'] = _to_log_scalar(pg.get('lr'))
                for k, v in loss_dict_reduced.items():
                    swanlab_payload[f'train/loss/{k}'] = _to_log_scalar(v)
                swanlab.log(
                    {k: v for k, v in swanlab_payload.items() if v is not None},
                    step=global_step,   
                )     
...
def evaluate(model: torch.nn.Module, criterion: torch.nn.Module, postprocessor, data_loader, coco_evaluator: CocoEvaluator, device, test_only=False, output_dir=None, yolo_metrice=False, other_platform_model=None):    
    ...
    is_coco_evaluator = (
        coco_evaluator is not None   
        and hasattr(coco_evaluator, "coco_eval")  
        and any(iou_type in ("bbox", "segm") for iou_type in iou_types)
    )    
    for samples, targets in metric_logger.log_every(data_loader, 1, header):
        ...
        if model is not None:
            ...
            if 'onnx' in other_platform_model:     
                with dt[0]:  
                    session = other_platform_model['onnx']
                    input_names = [inp.name for inp in session.get_inputs()]     
                    output_names = [out.name for out in session.get_outputs()]  
                    input_feed = build_runtime_input_feed(input_names, samples, orig_target_sizes, for_onnx=True) 
                    onnx_values = session.run(output_names=None, input_feed=input_feed)
                    outputs_by_name = dict(zip(output_names, onnx_values))
                    results = parse_runtime_outputs(outputs_by_name)
                        
            elif 'engine' in other_platform_model: 
                with dt[0]:  
                    engine_model = other_platform_model['engine']
                    input_feed = build_runtime_input_feed(     
                        engine_model.input_names, 
                        samples,
                        orig_target_sizes.to(device),
                        for_onnx=False,    
                    )   
                    output = engine_model(input_feed) 
                    results = parse_runtime_outputs(output)     
       
        res = {target['image_id'].item(): output for target, output in zip(targets, results)} # 将评估结果与图像 ID 关联
        if coco_evaluator is not None:
            _evaluator_update(coco_evaluator, res, targets) # 更新评估器    
            if is_coco_evaluator and 'bbox' in coco_evaluator.coco_eval:
                coco_det_pred_json.extend(list(coco_evaluator.coco_eval['bbox'].cocoDt.anns.values()))    
            if is_coco_evaluator and 'segm' in coco_evaluator.coco_eval:
    ...
    if yolo_metrice and is_coco_evaluator:   
    ...
    if coco_evaluator is not None:
        if is_coco_evaluator: 
            logger.info(RED + "------------------------ COCO Metrice Start ------------------------" + RESET)    
        coco_evaluator.accumulate()     
        coco_evaluator.summarize()  
        if test_only and is_coco_evaluator:
    ...
    stats = {}
    if coco_evaluator is not None:
        if is_coco_evaluator and 'bbox' in iou_types: 
            stats['coco_eval_bbox'] = coco_evaluator.coco_eval['bbox'].stats.tolist()
        if is_coco_evaluator and 'segm' in iou_types:    
            stats['coco_eval_masks'] = coco_evaluator.coco_eval['segm'].stats.tolist()
        if not is_coco_evaluator and hasattr(coco_evaluator, 'stats'):
...
def distill_one_epoch(self_lr_scheduler, lr_scheduler, model: torch.nn.Module, teahcer_model: torch.nn.Module, student_featureExt, teacher_featureExt,  
    ...
    for i, (samples, targets) in pbar:   
        ...
        with dt[4]:   
            ...
            if writer and dist_utils.is_main_process() and global_step % 10 == 0:   
                writer.add_scalar('Loss/total', loss_value.item(), global_step)
                if 'distill_decay' not in locals():
                    distill_decay = 1.0     
