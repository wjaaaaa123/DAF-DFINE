...
class DetSolver(BaseSolver):
    def __init__(self, cfg):  
        ...
        self.amp_fallback_failures = 0
        self.swanlab = None     
        self.swanlab_run_id = None
    ...
    def state_dict(self):
        ...
        if self.swanlab_run_id is not None:
            state['swanlab_run_id'] = self.swanlab_run_id     
        return state
    
    def load_state_dict(self, state): 
        super().load_state_dict(state)  
        self.swanlab_run_id = state.get('swanlab_run_id', None)
    ...
    def _swanlab_enabled(self):   
        yaml_cfg = getattr(self.cfg, 'yaml_cfg', {})
        if isinstance(yaml_cfg, dict) and 'use_swanlab' in yaml_cfg:
            return bool(yaml_cfg.get('use_swanlab', False))
        return bool(getattr(self.cfg, 'use_swanlab', False))     

    def _swanlab_cfg_value(self, name, default=None):     
        yaml_cfg = getattr(self.cfg, 'yaml_cfg', {})    
    ...
    @staticmethod
    def _parse_swanlab_config(cfg_str):    
        try:  
            return json.loads(cfg_str)
        except (TypeError, json.JSONDecodeError): 
            return cfg_str    

    def _build_swanlab(self, cfg_str, n_parameters, model_stats):
        ...
        try:    
            ...
            raise ImportError(
                "SwanLab visualization is enabled but the 'swanlab' package is not installed. "    
                "Install swanlab or disable --use-swanlab."
            ) from exc

        resume = 'must' if self.swanlab_run_id else None 
        run = swanlab.init(
            project=self._swanlab_cfg_value('swanlab_project', 'DEIM'),  
            name=self._swanlab_cfg_value('swanlab_run_name', None),
            id=self.swanlab_run_id,
            resume=resume,
            mode=self._swanlab_cfg_value('swanlab_mode', 'cloud') or 'cloud',   
            config={    
                'cfg': self._parse_swanlab_config(cfg_str),  
        ...
        if hasattr(swanlab, 'get_run'):     
            ...
            if active_run is not None:
                run = active_run
        self.swanlab_run_id = getattr(run, 'id', self.swanlab_run_id)
        return swanlab

    @staticmethod
    def _swanlab_scalar(value):
    ...
    def _swanlab_metric_names_for_key(self, key):   
        if key.endswith('coco_eval_bbox') or key.endswith('coco_eval_masks'):    
            return self._coco_metric_names()    
        return None
     
    @staticmethod
    def _swanlab_epoch_group(key):
    ...
    def _flatten_swanlab_epoch_metrics(self, log_stats):
        payload = {}   
        for key, value in log_stats.items():  
            metric_key = f'epoch/{self._swanlab_epoch_group(key)}/{key}'     
            scalar = self._swanlab_scalar(value)    
            if scalar is not None:    
                payload[metric_key] = scalar   
                continue

            if isinstance(value, (list, tuple)):    
                metric_names = self._swanlab_metric_names_for_key(key) or [] 
                for index, item in enumerate(value): 
                    item_scalar = self._swanlab_scalar(item)   
                    if item_scalar is None:
                        continue
                    suffix = metric_names[index] if index < len(metric_names) else f'metric_{index}'   
                    payload[f'{metric_key}_{suffix}'] = item_scalar
        return payload
     
    def _log_swanlab_epoch(self, log_stats, epoch):
        if self.swanlab is None or not dist_utils.is_main_process():
            return 
        payload = self._flatten_swanlab_epoch_metrics(log_stats)    
        if payload:
            self.swanlab.log(payload, step=epoch)
    ...
    @classmethod
    def _iter_numeric_metric_values(cls, values):
        ...
        for index, value in enumerate(iterator): 
            metric = cls._to_float_metric(value)
            if metric is not None:
                yield index, metric     

    @classmethod 
    def _first_numeric_metric(cls, values):   
        for _, metric in cls._iter_numeric_metric_values(values):    
            return metric
        return None

    def _coco_metric_names(self):
        evaluator = getattr(self, 'evaluator', None)    
        if getattr(evaluator, 'using_vtiny_metrice', False):   
            return coco_name_vtiny_list    
        if getattr(evaluator, 'using_tiny_metrice', False):     
            return coco_name_tiny_list 
    ...
    def _get_avg_ap_metric(test_stats):
        if not test_stats:
            return None
        metrics = []
        for values in test_stats.values():
            metric = DetSolver._first_numeric_metric(values)     
            if metric is not None:
    ...
    def _reset_epoch_state(self, epoch):
    ...
    def _apply_epoch_stage_transition(self, epoch):
    ...
    def _train_one_epoch_with_amp_rollback(self, epoch, args):
        amp_fallback_limit = 3   
        amp_fallback_disabled = (  
            args.use_amp_fallback_fp32   
            and self.amp_fallback_failures >= amp_fallback_limit   
        )
        if amp_fallback_disabled:     
            logger.warning(   
                YELLOW
                + (   
                    "AMP fallback has failed {} times; training epoch {} directly in FP32." 
                ).format(self.amp_fallback_failures, epoch) 
                + RESET
            )
        ...
        train_stats = train_one_epoch(
            ...
            use_amp_fallback_fp32=args.use_amp_fallback_fp32 and not amp_fallback_disabled,
            force_fp32=amp_fallback_disabled,
            swanlab=self.swanlab,     
        )  
     
        if not train_stats.get("_amp_epoch_failed", False):   
            return train_stats     
 
        self.amp_fallback_failures += 1

        rollback_path = self.output_dir / 'last.pth' if self.output_dir else None   
        if rollback_path is None or not rollback_path.exists():    
            raise RuntimeError(    
                "AMP fallback requested epoch rollback, but last.pth does not exist. "
                f"epoch={epoch}, step={train_stats.get('_failed_step')}, "
                f"reason={train_stats.get('_failed_reason')}"    
            )

        logger.warning(
            YELLOW
            + (   
                "AMP failed in epoch {} step {} ({}). "
                "Loading {} and retraining this epoch in FP32."   
            ).format(    
                epoch,
                train_stats.get('_failed_step'),
                train_stats.get('_failed_reason'),
                rollback_path,
            )
            + RESET
        )   
        ...
        with ModelSaverFunc(self):
            self.load_resume_state(str(rollback_path))

        self.amp_fallback_failures = max(self.amp_fallback_failures, 1)
        self._reset_epoch_state(epoch)   
        self._apply_epoch_stage_transition(epoch)   
    ...
    def _bootstrap_best_stat_from_eval(self, test_stats, epoch):   
        ...
        metrics = {
            ...
            if (metric := self._first_numeric_metric(values)) is not None  
        }  
        if not metrics:    
            logger.warning('Skip best_stat bootstrap because no numeric evaluation metric was found.')
            return     
    ...
    def fit(self, cfg_str):
        ...
        print(model_stats)
        self.swanlab = self._build_swanlab(cfg_str, n_parameters, str(model_stats))
        ...
        for epoch in range(start_epoch, args.epoches):
            ...
            # 训练一个 epoch
            train_stats = self._train_one_epoch_with_amp_rollback(epoch, args)   
            ...
            if self.output_dir and dist_utils.is_main_process():
                ...
                self._log_swanlab_epoch(log_stats, epoch)   
    ...
    def val(self, ):
        ...
        if self.output_dir:    
            self._save_eval_if_available(coco_evaluator)

        return

    def _save_eval_if_available(self, coco_evaluator):
        bbox_eval = getattr(coco_evaluator, "coco_eval", {}).get("bbox") if coco_evaluator is not None else None     
        eval_payload = getattr(bbox_eval, "eval", None)
        if eval_payload is None:  
            # logger.info("Skip saving eval.pth because evaluator has no bbox eval payload.")
            return False
     
        dist_utils.save_on_master(eval_payload, self.output_dir / "eval.pth")
        return True    
    ...
    def _load_engine_model(self, mode=None):  
        """加载TensorRT Engine模型"""   
        from tools.inference.detect.trt_inf import TRTInference   
     
        logger.info(f"Loading Engine Model: {self.cfg.path}")
        model = TRTInference(self.cfg.path, device=self.device, max_batch_size=1)     
    ...
    def save_best_model(self, test_stats, best_stat, epoch):     
        if test_stats:   
            ...
            all_metrics = []
            metric_names = []    
            coco_metric_names = self._coco_metric_names()
            for k, values in test_stats.items():  
                numeric_values = list(self._iter_numeric_metric_values(values))
                if self.writer and dist_utils.is_main_process():    
                    for i, v in numeric_values: 
                        metric_name = coco_metric_names[i] if i < len(coco_metric_names) else f'metric_{i}'
                        self.writer.add_scalar(f'Test/{k}_{metric_name}', v, epoch)
   
                if numeric_values:   
                    all_metrics.append(numeric_values[0][1]) # 0代表选择ap50-95 1代表选择ap50
                    metric_names.append(k) 
 
            if not all_metrics:
                logger.warning('Skip best model update because no numeric evaluation metric was found.')
                return best_stat     
            ...
            # Stage 2 开始时的特殊处理     
            elif getattr(self.train_dataloader.collate_fn, 'stop_epoch', None) is not None and epoch == self.train_dataloader.collate_fn.stop_epoch:
