...
class ResizePad(T.Transform): 
    ...
    def __init__(
        ...
        if pad_mode not in {'center', 'random'}: 
            raise ValueError(f'pad_mode must be center or random, got {pad_mode}') 
    ...
    def _sample_offset(self, delta: int) -> int:
        if delta <= 0:    
            return 0
        if self.pad_mode == 'center':
            return delta // 2   
        return int(torch.randint(0, delta + 1, (1,)).item())
    ...
    def make_params(self, flat_inputs: List[Any]) -> Dict[str, Any]:  
        ...
        left = self._sample_offset(pad_w)
        top = self._sample_offset(pad_h)   
        right = pad_w - left
        bottom = pad_h - top
    ...
    def _resize(self, inpt: Any, resize_size: Tuple[int, int]) -> Any: 
        if isinstance(inpt, (BoundingBoxes, Mask)):     
            return F.resize(     
                inpt,    
                size=resize_size,
    ...
    def _pad(self, inpt: Any, padding: List[int]) -> Any:  
        return F.pad(inpt, padding=padding, fill=self.fill, padding_mode=self.padding_mode)  # type: ignore[arg-type] 

    def _transform(self, inpt: Any, params: Dict[str, Any]) -> Any:
        resized = self._resize(inpt, params['resize_size'])
        return self._pad(resized, params['padding'])
    ...
    def __call__(self, *inputs: Any) -> Any:
        ...
        if len(outputs) > 1 and isinstance(outputs[1], dict):
            outputs[1]['padding'] = torch.tensor(self.padding) 
            outputs[1]['resize_pad_size'] = torch.tensor(self.size) 
        return outputs
